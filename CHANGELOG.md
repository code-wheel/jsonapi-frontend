...
- Document the public-endpoint access model in `SECURITY.md`: `/jsonapi/resolve`
  and `/jsonapi/routes` use the JSON:API core `_access: TRUE` pattern with
  authorization enforced in code (entity view access and a shared secret,
  respectively). No behavior change; references existing denial-path tests.
...
- `PathResolverInterface` to support optional resolver integrations (for example `jsonapi_frontend_webform`)
